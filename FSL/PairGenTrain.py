class PairGenTrain:
    def __init__(self, X_support, y_support, X_train, y_train, X_valid, y_valid, batch_size=32, step=256, positive_label=1.0, negative_label=0.0):
        import numpy as np
        ...
        def generator_train():
            ...
            while step_cnt < step:
                ...
                while batch_cnt < batch_size:
                    ...
                    if current_label == 1:
                        classNum = keyList[np.random.randint(len(train_idx))]
                        indexArr = [np.random.randint(len(support_idx[classNum])), np.random.randint(len(train_idx[classNum]))]
                        ...
                        classNumIndex = np.random.choice(len(train_idx), 2, replace=False)
                        classNum = [keyList[classNumIndex[0]], keyList[classNumIndex[1]]]
                        indexArr = [np.random.randint(len(support_idx[classNum[0]])), np.random.randint(len(train_idx[classNum[1]]))]
        ...
        def generator_valid():
            ...
            while step_cnt < step:
                ...
                while batch_cnt < batch_size:
                    ...
                    current_label = np.random.randint(2)
                    if current_label == 1:
                        classNum = keyList[np.random.randint(len(valid_idx))]
                        indexArr = [np.random.randint(len(support_idx[classNum])), np.random.randint(len(valid_idx[classNum]))]
                        image_left.append(X_support[support_idx[classNum][indexArr[0]]])
                        image_right.append(X_valid[valid_idx[classNum][indexArr[1]]])
                        label.append([positive_label])
                    else:
                        classNumIndex = np.random.choice(len(valid_idx), 2, replace=False)
                        classNum = [keyList[classNumIndex[0]], keyList[classNumIndex[1]]]
                        indexArr = [np.random.randint(len(support_idx[classNum[0]])), np.random.randint(len(valid_idx[classNum[1]]))]
                        image_left.append(X_support[support_idx[classNum[0]][indexArr[0]]])
